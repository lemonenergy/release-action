...
const getLastVersion = async (base2, initial = "0.0.0", targetPath2 = "") => {
  try {
...
const bump = async (lastVersion, release, targetPath2 = "") => {
  const version = inc(lastVersion, release);
  if (targetPath2) {
    await exec(`ls /usr/bin`);
    await exec(`cd ${targetPath2}`);
  }
  await exec(
...
const configGit = async (head2) => {
  await exec(`git fetch ${remote} ${head2}:${head2}`);
  await exec(`git config --local user.email "action@github.com"`);
  await exec(`git config --local user.name "Version Bump Action"`);
  await exec(`git checkout ${head2}`);
};
const pushBumpedVersionAndTag = async (head2) => {
  await exec(`git push "${remote}" HEAD:${head2}`);
