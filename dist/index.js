    ...
    var SemVer = class {
      constructor(version2, options) {
        ...
        if (version2 instanceof SemVer) {
          ...
          throw new TypeError(`Invalid Version: ${__require("util").inspect(version2)}`);
        }
...
var require_esm = __commonJS({
  "node_modules/esm/esm.js"(exports, module) {
    ...
    var f = __require("module");
    var { Script: m } = __require("vm");
    var { createCachedData: y, runInNewContext: h, runInThisContext: b } = m.prototype;
    var { sep: g } = __require("path");
    var { readFileSync: v } = __require("fs");
    var w = new f(s);
    function q(e2) {
      let t2;
      try {
...
// node_modules/recommended-bump/package.json
var require_package = __commonJS({
  "node_modules/recommended-bump/package.json"(exports, module) {
    module.exports = {
...
// node_modules/recommended-bump/index.js
var require_recommended_bump = __commonJS({
  "node_modules/recommended-bump/index.js"(exports, module) {
    "use strict";
    var path = __require("path");
    var esmLoader = require_esm();
    var pkg = require_package();
    var esmRequire = esmLoader(module);
    function interop(x) {
    ...
    var mod = esmRequire(path.join(__dirname, pkg.module));
...
var import_recommended_bump = __toESM(require_recommended_bump(), 1);
var import_core = __toESM(require_core(), 1);
var import_github = __toESM(require_github(), 1);
var import_exec = __toESM(require_exec(), 1);
var import_rest = __toESM(require_dist_node20(), 1);
import { readFileSync } from "fs";
import { exit } from "process";
var EVENT = "pull_request";
var githubToken = (0, import_core.getInput)("github-token");
var base = (0, import_core.getInput)("base-branch");
var head = (0, import_core.getInput)("head-branch");
var initialVersion = (0, import_core.getInput)("initial-version");
var targetPath = (0, import_core.getInput)("path");
var actor = process.env.GITHUB_ACTOR;
var repository = process.env.GITHUB_REPOSITORY;
var remote = `https://${actor}:${githubToken}@github.com/${repository}.git`;
var octokit = new import_rest.Octokit({ auth: githubToken });
var { context } = import_github.default;
var checkEvent = () => {
...
var validatePullRequest = async () => {
  ...
  if (!pull_request.mergeable)
    throw Error(`PR isn't mergeable`);
};
var validateCommitMessage = (message) => {
...
var getPullRequestCommits = async (parameters) => {
  const commits = [];
...
var getRelease = async () => {
  ...
  const messages = commits.map(({ commit }) => commit.message).filter(validateCommitMessage);
  const { increment: release } = (0, import_recommended_bump.default)(messages);
  return release;
};
var bump = async (lastVersion, release, targetPath2 = "") => {
  const version2 = (0, import_semver.inc)(lastVersion, release);
  if (targetPath2) {
    await (0, import_exec.exec)(`ls /usr/bin`);
    await (0, import_exec.exec)(`cd ${targetPath2}`);
  }
  await (0, import_exec.exec)(
    `npm version --new-version ${version2} --allow-same-version -m "Release v%s"`
  );
  const file = readFileSync(`${targetPath2 ? `${targetPath2}/` : ""}package.json`);
  const { version: bumped } = JSON.parse(file.toString());
  return bumped;
};
var configGit = async (head2) => {
  await (0, import_exec.exec)(`git fetch ${remote} ${head2}:${head2}`);
  await (0, import_exec.exec)(`git config --local user.email "action@github.com"`);
  await (0, import_exec.exec)(`git config --local user.name "Version Bump Action"`);
  await (0, import_exec.exec)(`git checkout ${head2}`);
};
var pushBumpedVersionAndTag = async (head2) => {
  await (0, import_exec.exec)(`git push "${remote}" HEAD:${head2}`);
  await (0, import_exec.exec)(`git push -f --tags`);
};
try {
  ...
  if (!release) {
    (0, import_core.warning)("no release needed!");
    exit(0);
  }
  ...
  const version2 = await bump(lastVersion, release, targetPath);
  console.log(`bumped to version ${version2}!`);
  await pushBumpedVersionAndTag(head);
  console.log(`version ${version2} pushed!`);
  (0, import_core.setOutput)("version", version2);
} catch (e) {
